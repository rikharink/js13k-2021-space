...
interface GamepadState {
  axesStatus: number[];
  buttonsStatus: (GamepadButton | undefined)[];
  axesCache: number[];
  buttonsCache: (GamepadButton | undefined)[];
}

export class GamepadManager
  extends EventEmitter<GamepadManagerEvent>
  implements Disposable, ITickable {
  ...
  public vibrate(gamepadIndex: number, effect: GamepadEffect): void {
    const gamepad = this.getGamepadByIndex(gamepadIndex);
    if (!gamepad || !gamepad.vibrationActuator) return;
    gamepad.vibrationActuator.playEffect('dual-rumble', effect);
  }

  public getGamepadByIndex(index: number): RawGamepad | null {
  ...
  private updateGamepadState(index: number, state: GamepadState) {
    ...
    let pressed: (GamepadButton | undefined)[] = [];
    if (gamepad.buttons) {
